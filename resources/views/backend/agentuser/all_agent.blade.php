...
<div class="card">
  <div class="card-body">
          ...
          <tr>
            ...
            <td>
              <img src="{{ (!empty($item->photo)) ? 
                  asset('upload/agent_images/'.$item->photo) : 
                  url('upload/no_image.jpg') }}"
              style="width: 60px; height: 55px;" alt="">
          </td>
    ...
    <input data-id="{{ $item->id }}" class="toggle-class" 
    ...
    data-on="Active" data-off="Inactive"{{$item->status?'checked':''}}>
            ...
            <td>
              {{-- <a href="{{route('details.property',$item->id)}}" class="btn btn-inverse-info" id="details" title="Details"><i data-feather="eye"></i></a>  --}}
              <a href="{{route('edit.agent',$item->id)}}" class="btn btn-inverse-warning" id="edit" title="Edit"><i data-feather="edit"></i></a>  
